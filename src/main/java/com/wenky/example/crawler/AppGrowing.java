...
public class AppGrowing {
  ...
  public void preCheck(String cookie, String referer) {
    ...
    ResponseEntity<String> response =
        restTemplate.exchange(
            url, HttpMethod.GET, fetchEntity(cookie, referer, "same-site"), String.class);
    String url1 = "https://ds.appgrowing.cn/api/nps";
    ResponseEntity<String> response1 =
        restTemplate.exchange(
            url1, HttpMethod.GET, fetchEntity(cookie, referer, "same-origin"), String.class);
  }
  ...
  public List<String> fetchIdList(Integer page, Integer pageSize) {
    ...
    ResponseEntity<String> response =
        restTemplate.exchange(
            url, HttpMethod.GET, fetchEntity(cookie, referer, "same-origin"), String.class);
      ...
      redirectUrl =
          Optional.ofNullable(response.getBody())
              ...
              .map(
                  array ->
                      array.stream()
                          .map(json -> ((JSONObject) json).getString("redirect"))
                          .findFirst()
                          .orElse(null))
              .orElse(null);
    } catch (Exception e) {
