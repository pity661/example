...
public class mixige {
  ...
  private static void sheet0() {
    ...
    try (FileInputStream inputStream = new FileInputStream("/Users/huwenqi/Desktop/墨西哥贷超码表.xlsx")) {
      ...
      list.stream()
          .forEach(
              single -> {
                String s = String.format(formatString, single.get(6), single.get(1), single.get(3));
      ...
      list.stream()
          .forEach(
              single -> {
                String s =
                    String.format(
                        formatString1, single.get(2), single.get(3), single.get(0), single.get(1));
                System.out.println(s);
              });
      list.stream()
          .forEach(
              single -> {
                String s =
                    String.format(formatString2, single.get(6), single.get(0), single.get(2));
                System.out.println(s);
              });
    } catch (Exception e) {
      e.printStackTrace();
